...
public class FieldsTest {
	private class TestOptions {
		...
		[Desc("Description on boolean")]
		public bool Checkbox { get; set; }
		[Switch] public bool ReadonlySwitch { get; private set; } = true;

		[Desc("Description on enum")]
		public TestEnum DropdownValue { get; set; } = TestEnum.FirstVariant;
		[ReadOnly]
		public TestEnum ReadonlyDropdownValue { get; set; } = TestEnum.SecondVariant;
	...
	private enum TestEnum {
		...
		FirstVariant,
		[Desc("Description on enum variant")]
		SecondVariant,
		ThirdVariant,
		ForthVariant = 4
	}
	...
	public void TestAnalyzedFields() {
		...
		{ // Testing bitmask
			...
			Assert.That(bitmaskField.Variants[1].Item2, Is.EqualTo("This is two"), $"Field '{title}' 2nd variant had invalid description");
	...
	private static void AssertLabel(IEnumerator<Field> enumerator, string title, string text, string? description = null) {
	...
	private static void AssertStringInput(IEnumerator<Field> enumerator, string title, string text, bool readOnly, string? description = null) {
	...
	private static void AssertCheckbox(IEnumerator<Field> enumerator, string title, bool value, bool readOnly, bool switchStyle, string? description = null) {
		...
		if (readOnly) Assert.That(checkbox.Value, Is.EqualTo(value), $"Field '{title}' updated its value despite being read-only");
	...
	private static void AssertEnum(IEnumerator<Field> enumerator, string title, string currentValue, string newValue, bool readOnly, string? description = null, string? valueDescription = null) {
		...
		if (readOnly) Assert.That(enumField.Value, Is.EqualTo(currentValue), $"Field '{title}' updated its value despite being read-only");
		else Assert.That(enumField.Value, Is.EqualTo(newValue), $"Field '{title}' didn't update its value after being set");
	}
}