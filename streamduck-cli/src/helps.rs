pub const COMMANDS: &'static [(&'static str, &'static str)] = &[
    ...
    ("(br)ightness", "[<serial>] <0-100> - sets brightness for device"),
    ("(ba)ck", "[<serial>] [drop] - navigates back, even if there's no button for that, if drop is included, goes to root screen"),
